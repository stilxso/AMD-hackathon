...
>>>>>>> 0b88e6d (Complete Stage 3, 4, 5: Fusion engine, API clients, Mapbox frontend, Documentation)
  ...
  useEffect(() => {
    ...
    // We only create the map once
    mapRef.current = new mapboxgl.Map({
      container: mapContainerRef.current,
      style: "mapbox://styles/mapbox/dark-v11",
      center: [coords.longitude, coords.latitude], // Mapbox uses [lng, lat]
      zoom: 13,
    });
  ...
  // Update center and marker when coords/aqi change
  useEffect(() => {
    ...
    const el = document.createElement("div");
    el.className = "airq-pin-wrapper";
    el.innerHTML = `<div class="airq-pin" style="background: radial-gradient(circle at 30% 30%, #d1fae5, ${hex} 55%, #064e3b 100%); box-shadow: 0 0 0 4px ${hex}30, 0 0 22px 6px ${hex}80; width: 22px; height: 22px; border-radius: 50%;"></div>`;
    
    const popupContent = `
      <div style="font-family: system-ui; color: #0b3b26; padding: 4px;">
        <div style="font-weight: 600;">${t.yourLocation}</div>
        <div style="font-family: ui-monospace, monospace; font-size: 12px; margin-top: 2px;">
          ${lat.toFixed(4)}&deg;, ${lng.toFixed(4)}&deg;
        </div>
        ${
          aqi != null
            ? `<div style="margin-top: 6px; font-size: 12px;">${t.aqi}: <strong>${aqi}</strong></div>`
            : ""
        }
      </div>
    `;
